...
- where:
  ...
  - `vars` is an optional character vector of variables to describe. If `NULL` (default), all columns except `cluster_col` are used. 
  ...
  - `auto_print` is logical; if TRUE (default), prints summaries to the console.
  
Using the results from above, the output highlights which variables significantly differentiate clusters and provides detailed summaries only where differences are statistically meaningful. Note that if we do not indicate which `vars` we want to use, the amount of results is usually very large.

```{r hc_describe}
describe_vars <- c("usertype", "spend", "hours", "eatin", "mealplan", "gender")
